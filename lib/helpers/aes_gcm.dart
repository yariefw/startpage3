...
class AesGcm {
  static Future<Uint8List> hashKey(String plaintext) async {
    final hash = await cryptography.Sha256().hash(utf8.encode(plaintext));
  ...
  static Future<String> encrypt(
    ...
    if (key.length == 32) {
      // Legacy input, key needs to be exactly 32 bytes
      gcmKey = utf8.encode(key);
    } else {
      // New input, key can be any size
      gcmKey = await hashKey(key);
    }
